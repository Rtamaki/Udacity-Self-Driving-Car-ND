...
os.chdir('/home/tamaki/Documents/Udacity/CarND-Semantic-Segmentation/CarND-Semantic-Segmentation')
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
   ...
   # with tf.device("/gpu:0"):
    ...
    conv_1x1_7 = tf.layers.conv2d(vgg_layer7_out, num_classes, 1, padding='same', 
                                kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3), #regulizer is important to avoid overfitting    
    ...
    conv_1x1_4 = tf.layers.conv2d(vgg_layer4_out, num_classes, 1, padding='same', 
                                kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3),#regulizer is important to avoid overfitting
                                kernel_initializer= tf.random_normal_initializer(stddev=0.01)) 
    add_4 = tf.add(output_7, conv_1x1_4)
    output_4 = tf.layers.conv2d_transpose(add_4, num_classes, 4, 2, padding='same',
                                        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3), 
                                        kernel_initializer= tf.random_normal_initializer(stddev=0.01))
    ...
    conv_1x1_3 = tf.layers.conv2d(vgg_layer3_out, num_classes, 1, padding='same', 
                                kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3),#regulizer is important to avoid overfitting
                                kernel_initializer= tf.random_normal_initializer(stddev=0.01)) 
    add_3 = tf.add(output_4, conv_1x1_3)
    output = tf.layers.conv2d_transpose(add_3, num_classes, 16, 8, padding='same',
                                        kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3),
                                        kernel_initializer= tf.random_normal_initializer(stddev=0.01))
...
#%%
   ...
   #  with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
    with tf.Session() as sess:
        ...
        w1, keep, w3, w4, w7 = load_vgg(sess, vgg_path)
        last_layer = layers(w3, w4, w7, num_classes)
        logits, train_op, cross_entropy_loss = optimize(last_layer, correct_label, learning_rate, num_classes)

        # TODO: Train NN using the train_nn function
        train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, w1,
